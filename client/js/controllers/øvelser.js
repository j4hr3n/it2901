...
function exerciseCtrl($scope) {
  ...
  $scope.addExercise = function(userId) {
    ...
    if (userId === Meteor.userId()) {
      ...
      sweetAlert("Oops...", "You are already friend with this person!",
        "error");
    } else if (isRequest(userId) == true) {
      sweetAlert("Oops...",
        "You have already sent friend invitation to this person!", "error");
      ...
      if (reqList.indexOf(userId) < 0) {
        ...
        swal("Oops",
          "You've already got an invitation from this freind. You just need to approve it!",
          "error")
      }
